    ...
    <div class="flex flex-col w-full mt-3 lg:ml-3 lg:mt-2">
      ...
      <a-spin :loading="resourceLoading" :tip="$t('maResource.loadingText')" class="h-full">
        <div class="mt-2 resource-list" ref="rl" v-if="attachmentList && attachmentList.length > 0">
          <div class="rounded-sm item" v-for="(item, index) in attachmentList" :key="item.hash"
            @click="selectFile(item, index)">
            <img :src="! /^(http|https)/g.test(item.url)
      ...
      <div class="lg:flex lg:justify-between">
        <a-pagination :total="pageInfo.total" v-model:current="pageInfo.currentPage" v-model:page-size="pageSize"
          @change="changePage" />
        <div>
          <a-button type="primary" status="warning" @click="selectDownload" class="mt-3 mr-3 lg:mt-0">下载选择文件</a-button>
