		...
		<a-card size="small">
			<template #title>
				<div class="flex flex-row align-center">
					<a-checkbox v-if="scene !== 'index'" :value="order.id"
						><span v-if="scene !== 'create'"
							>订单编号:<span class="pl-2">{{ order.code }}</span></span
						></a-checkbox
					>
					<div class="text-xs">
						<span class="pr-4 font-bold text-center" v-if="scene === 'index' && order.store_name"><span class="pr-2">订单编号:</span>{{ order.code }}</span>
						...
						<consignee-select
							...
							:allow-diff-store="props.allowDiffStore"
						/>
						<span class="pl-2"><a-input :disabled="true" :readonly="true" size="mini" v-model="order.consignee_address" style="width: 500px" /></span>
					</div>
				</div>
			</template>
			<template #extra>
				...
				<a-button type="primary" v-if="scene === 'check' && identity !== 'store'" status="success" size="mini" @click="handleSubmitCheck" style="margin-right: 1rem"
					>发起提交</a-button
				>
				<a-button type="primary" v-if="order.status < 50 && identity === 'client'" status="danger" size="mini" @click="handleDeleteOrder">删除订单</a-button>
			</template>
			...
			<div class="flex flex-row gap-4 pb-3 align-center">
				...
				<div class="flex flex-row gap-2 align-center">
					<span v-if="scene === 'check'" class="font-bold" style="font-size: 12px; padding-top: 0.125rem; color: black"
						>来源:
			...
			<a-table size="mini" :data="goods" column-resizable :bordered="{ cell: true }" :pagination="false" ref="tableRef" hide-expand-button-on-empty :scroll="{ x: 2000, y: 2000 }">
				<template #columns>
					...
					<a-table-column title="画面类型" data-index="product_picture_type_id" :width="90">
						...
						<!--          </template>-->
						<template #cell="{ record, column, index }">
							<span v-if="bizDict.$state['productPictureType']">{{ renderDictValue('productPictureType', record.product_picture_type_id) }}</span>
							<span v-else>加载中</span>
						</template>
					</a-table-column>
					...
					<a-table-column title="工艺" data-index="craft_id" :width="110">
						<template #cell="{ record, column, index }">
							...
							<pricing-craft-select
								size="mini"
								v-if="record.row_type === 'craft'"
								v-model="record.craft_id"
								:disabled="disableCraftEdit(record)"
								@change="onCraftChanged($event, record)"
...
function onCancelGoodsOrCraft(record) {
	if (typeof record.key === 'string' && record.key.includes('tmpAdd')) {
		if (record.row_type === 'goods') {
			goods.value.splice(
				goods.value.findIndex((item) => item.key === record.key),
				1
			)
		} else if (record.row_type === 'craft') {
			const parent = goods.value.find((item) => item.id === record.parent_id)
			parent.children.splice(
				parent.children.findIndex((item) => item.key === record.key),
				1
			)
		} else {
			console.error('未知类型')
