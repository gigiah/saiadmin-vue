      ...
      <template #title>
        <div class="flex flex-row align-center">
          ...
          <div class="text-xs">
            ...
            <span class="pr-4 font-bold text-center" v-if="order.store_product_type">{{ order.store_product_type
              }}</span>
            <span class="pr-4 font-bold text-center" v-if="order.store_pricing_type">{{ order.store_pricing_type
              }}</span>
            ...
            <consignee-select style="width: 150px" v-model="order.consignee_id" :storeId="order.store_id"
              :isDisabled="scene === 'index'" size="mini" @change="onStoreChanged" />
            <span class="pl-2">
      ...
      <a-table size="mini" :data="goods" column-resizable :bordered="{ cell: true }" :pagination="false" ref="tableRef"
        ...
        <template #columns>
          ...
          <a-table-column title="数量" data-index="nums" :width="120">
            <template #cell="{ record, column, index }">
              <a-input-number size="mini" v-model="record.nums" :disabled="scene !== 'index' || !record.editable"
                :min="0" :step="1" />
            </template>
          </a-table-column>
          ...
          <a-table-column title="单价" data-index="unit_price">
            <template #cell="{ record, column, index }">
              <a-input v-model="record.unit_price"
                :disabled="record.pricing_type_id !== 9 || scene === 'index' || !record.editable" />
            </template>
            <!-- <template #cell="{ record, column, index }">
