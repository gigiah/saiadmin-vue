			...
			<template #title>
				<div class="flex flex-row align-center">
					...
					<div class="text-xs">
						...
						<span class="pl-2">
							<a-input :disabled="true" :readonly="true" size="mini" v-model="order.consignee_address" style="width: 500px" />
						</span>
					</div>
				</div>
			</template>
          ...
          @change="onPayStatusChange" /> -->
			...
			<div class="flex flex-row gap-4 pb-3 align-center">
				...
				<div class="flex flex-row gap-2 align-center">
					...
					<a-input size="mini" v-model="order.remark" style="width: 500px" @focus="onRemarkFocus" @blur="onRemarkBlur" @pressEnter="onRemarkBlur" />
			...
			<a-table size="mini" :data="goods" column-resizable :bordered="{ cell: true }" :pagination="false" ref="tableRef" hide-expand-button-on-empty :scroll="{ x: 2000, y: 2000 }">
				<template #columns>
					...
					<a-table-column title="工艺" data-index="craft_id" :width="110">
						<template #cell="{ record, column, index }">
							<a-button v-if="record.row_type === 'goods' && scene === 'confirm'" size="mini" status="success" shape="circle" @click="addCraft(record)">
								<icon-plus />
							</a-button>
							<pricing-craft-select
								...
								:search-all-range="true"
								:client-group-id="record.client_group_id"
							/>
						</template>
					</a-table-column>
