	...
	<div class="justify-between p-4 ma-content-block lg:flex">
		...
		<ma-crud :options="crud" :columns="columns" ref="crudRef">
			...
			<template #tableAfterButtons>
				<a-button v-if="sysInfoStore.info.is_team_leader" type="outline" @click="modifyShow"
					><template #icon><icon-code /></template>选择产品
				</a-button>
			</template>
		</ma-crud>
		<a-modal v-model:visible="visible" @ok="modifySave">
			<a-transfer
				...
				show-search
				:data="items4Transfer"
				v-model="items4TransferSelected"
...
const getProducts = () => {
	...
	requestApi
		.getSelectList({ type: 'all', scene: 'market' })
		.then((res) => {
